...
def _run_scrapy(source_url: str, out_jsonl: str, config: dict[str, Any]) -> None:
    ...
    # NOTE: _out_file is opened INSIDE the try block below (WR-06) so that the
    # finally clause can always close it — even if an exception is raised during
    # class body definitions (MaxPagesExtension / KlakeSpider) or during
    # CrawlerProcess setup. The .closed check in finally avoids double-close when
    # KlakeSpider.closed() already flushed and closed the file.
    _out_file = None
