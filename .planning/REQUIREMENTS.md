...
- [x] **STORE-03**: The gold zone is segmented by domain and dataset type — `gold/{domain}/rag_corpus/`, `gold/{domain}/pretrain/`, `gold/{domain}/finetune/`.
...
- [x] **SCHED-01**: A Dagster sensor triggers periodic re-crawl of a source based on its `crawl_schedule`, using a deterministic `run_key` and a cursor watermark to avoid duplicate runs and tick storms.
- [x] **SCHED-02**: On re-crawl, a content-change comparison over the **normalized silver-stage text** (not raw bytes) decides whether to re-ingest, so dynamic timestamps/nonces don't thrash the immutable raw zone; a max-staleness threshold forces an occasional refresh to catch false negatives.
