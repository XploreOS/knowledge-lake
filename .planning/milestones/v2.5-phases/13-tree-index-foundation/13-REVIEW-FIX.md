...
**Commit:** 2a44291
**Applied fix:** Changed `scope="global"` to `scope="tree_index"` in both the budget check (`registry_repo.get_llm_spend`) and the spend recording (`registry_repo.record_llm_spend`) calls inside `tree_index()`. This isolates tree-index LLM spend from enrichment spend so `settings.tree.budget_usd` applies only to tree-index operations. Tests updated in iteration 2 (commit `9726325`) to seed and assert against `scope="tree_index"`.
...
## Iteration 2 Fixes (commit `9726325`)

### Test scope correction (test_tree_index.py)

**Files modified:** `tests/unit/test_tree_index.py`
- Line 452: `scope="global"` → `scope="tree_index"` in happy-path spend assertion
- Line 467: `scope="global"` → `scope="tree_index"` in budget-exceeded seed
- Line 173: `in ("indexed", "tree_indexed", "complete")` → `== "tree_indexed"` (tighten IN-01 phantom values)

**Test result:** 567 unit tests passing, 0 failures.

## Remaining Issues (not in fix scope — advisory)

- **WR-01:** `PageIndexIndexer.build_index` returns `roots=[]` for no-section docs (pipeline path returns synthetic root). Low priority — direct plugin callers only.
- **WR-02:** Dead `hasattr(s, "tree")` guard + unused `_TREE_SCHEMA_VERSION` constant in `tree_index.py`. Dead code only.
- **WR-03:** `_summarize_nodes_llm` reads cost constants from `s.enrich` namespace. Cost-accounting coupling only.
- **IN-01:** `TreeNode.summary` docstring inaccurate ("empty in deterministic mode" — always set to heading).
- **IN-02:** `PageIndexIndexer.build_index` returns `content_hash=""` without docstring warning.
